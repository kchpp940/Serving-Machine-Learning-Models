    ...
    try:
        ...
        sorted_features = sorted(
            [
                {
                    FIELD_FEATURE: f,
                    FIELD_LABEL: model.schema.label(f),
                    FIELD_GLOBAL_IMPORTANCE: score,
                    FIELD_GLOBAL_IMPORTANCE_PERCENT: round(score * 100, 2),
                }
                for f, score in importances.items()
            ],
            key=lambda x: x[FIELD_GLOBAL_IMPORTANCE],
            reverse=True,
        )
        return {
            "model_name": model.model_name,
...
def predict(data: CarPrediction):
    try:
        model = get_model()
        result = model.predict_from_pydantic(data)
        return PredictionResponse.from_protocol(result)
...
def explain_prediction(
    ...
    try:
        ...
        return ExplainResponse.from_protocol(result)
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
    except HTTPException:
        raise
