...
def _car_pricing_api_dir() -> Path:
    return REPO_ROOT / "car_pricing_api"


def _streamlit_app_path() -> Path:
    return REPO_ROOT / "streamlitapp" / "streamlit_app.py"


def _legacy_fastapi_dir() -> Path:
    return REPO_ROOT / "fastapi"


# ---------------------------------------------------------------------------
# 1. Artifact checks — reuse ModelService for paths and loading
# ---------------------------------------------------------------------------

def check_artifact_paths(report: CheckReport) -> None:
    api_dir = _car_pricing_api_dir()
    ...
    try:
        ...
        ModelService = services_mod.ModelService
        service = ModelService(model_dir=str(api_dir / "models"))
...
def check_model_loadable(report: CheckReport) -> None:
    ...
    api_dir = _car_pricing_api_dir()
    model_path = api_dir / "models" / "sklearn_gbr.pkl"
    ...
    try:
        model = CarPriceModel.from_joblib(str(model_path))
...
# ---------------------------------------------------------------------------
# 2. Package import boundary checks
# ---------------------------------------------------------------------------
...
def check_third_party_fastapi_not_shadowed(report: CheckReport) -> None:
    legacy_dir = _legacy_fastapi_dir()
    ...
    try:
        ...
        shadow_path = REPO_ROOT / "fastapi" / "__init__.py"
        if repo_root_str in sys.path:
            ...
            try:
                ...
                if would_shadow:
                    ...
                    report.add(CheckResult(
                        name="import.fastapi_shim_risk",
                        passed=True,
                        message=f"即使 repo 根在 sys.path[0], fastapi 仍解析到第三方包",
                    ))
            except Exception as e:
                report.add(CheckResult(
                    name="import.fastapi_shim_risk",
...
def check_legacy_shim_path_manipulation(report: CheckReport) -> None:
    legacy_app = _legacy_fastapi_dir() / "app.py"
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.Call):
                ...
                if (isinstance(func, ast.Attribute)
                        ...
                        and func.attr == "path"
                        and isinstance(getattr(func, "ctx", None), ast.Load)):
                    pass
                if (isinstance(func, ast.Attribute)
...
def check_fastapi_app_imports(report: CheckReport) -> None:
    api_dir = _car_pricing_api_dir()
...
def check_streamlit_no_hardcoded_schema(report: CheckReport) -> None:
    ...
    try:
            ...
            if isinstance(node, ast.ImportFrom):
                if node.module and node.module.startswith("car_pricing"):
                    for alias in node.names:
                        if "client" in alias.name.lower() or "api" in alias.name.lower():
                            imports_shared_client = True
            elif isinstance(node, ast.Import):
                for alias in node.names:
                    if alias.name in ("requests",):
                        uses_requests_directly = True
                    if alias.name.startswith("car_pricing"):
                        imports_shared_client = True
            ...
            if isinstance(node, ast.Assign):
                ...
                for value in ast.walk(node):
                    if isinstance(value, ast.Constant) and isinstance(value.value, str):
                        _scan_string_for_endpoints(value.value)
        ...
        shared_client_used = imports_shared_client and not uses_requests_directly
        ...
        if shared_client_used:
            client_msg = "Streamlit 通过 car_pricing 共享 client 进行 API 调用"
        elif uses_requests_directly:
            client_msg = "Streamlit 直接使用 requests 调用 API (import requests as re), 建议抽离共享 client 统一维护端点与契约"
        else:
            client_msg = "Streamlit 未检测到 requests 调用，也未使用共享 client"
        report.add(CheckResult(
            name="frontend.shared_client",
            passed=shared_client_used,
            message=client_msg,
            details={
...
ALL_CHECKS: List[Callable[[CheckReport], None]] = [
    ...
    check_fastapi_app_imports,
    check_pydantic_schema_consistency,
    check_streamlit_no_hardcoded_schema,
]
